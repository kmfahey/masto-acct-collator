...
connection = MySQLdb.Connect(
    ...
    password=None,
    host="localhost",
    database="mst_acct_recdr",
    charset="utf8mb4",
)
cursor = connection.cursor()

userids_follow_ing_ers = set(pickle.load(open("followers.dat", "rb"))) & set(
    pickle.load(open("following.dat", "rb"))
)
...
for term in terms:
    fulltext_clause = " OR ".join(
        "MATCH({field}) AGAINST('{term}'{mode})".format(
            field=field,
            term=term,
            mode=" IN BOOLEAN MODE" if field != "user_id" else "",
        )
        for field in fields
    )
    rows_affected = cursor.execute(
        f"SELECT id, acct_id, user_id, user_name, url, tested FROM profiles WHERE {fulltext_clause};"
    )
    for _id, acct_id, user_id, user_name, url, tested in cursor.fetchall():
        if tested == 1:
            continue
        result_rows.add(tuple(map(str, (_id, acct_id, user_id, user_name, url))))
    # print(f"added {rows_affected} rows_affected for search against {term}")
    # print(f"running total rows_affected = {len(result_rows)}")

result_rows = list(
    filter(lambda row: row[2] not in userids_follow_ing_ers, result_rows)
)
...
row_str_w_padding = (
    lambda id_, acct_id, user_id, user_name, url: "| "
    + " | ".join(
        (
            id_.ljust(max_id_width),
